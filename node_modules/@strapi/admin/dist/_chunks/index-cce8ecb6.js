...
const syncFs__default = /* @__PURE__ */ _interopDefault(syncFs);
...
const build$2 = ({ command, ctx }) => {
  command.command("build").option("-d, --debug", "Enable debugging mode with verbose logs", false).option("--ignore-prompts", "Ignore all prompts", false).option("--minify", "Minify the output", true).option("--no-optimization", "[deprecated]: use minify instead").option("--silent", "Don't log anything", false).option("--sourcemap", "Produce sourcemaps", false).option("--stats", "Print build statistics to the console", false).description("Build the strapi admin app").action(async (options) => {
    const { build: build2 } = await Promise.resolve().then(() => require("./build-55ba3e68.js"));
...
const develop$1 = ({ command, ctx }) => {
  command.command("develop").alias("dev").option("-d, --debug", "Enable debugging mode with verbose logs", false).option("--silent", "Don't log anything", false).option("--ignore-prompts", "Ignore all prompts", false).option("--polling", "Watch for file changes in network directories", false).option("--watch-admin", "Watch the admin panel for hot changes", false).option(
    ...
    const { develop: develop2 } = await Promise.resolve().then(() => require("./develop-69e28e61.js"));
...
const getEntryModule = ({ plugins }) => {
  const pluginsObject = plugins.map(({ name, importName }) => `'${name}': ${importName}`).join(",\n");
  const pluginsImport = plugins.map(({ importName, path: path2 }) => `import ${importName} from '${path2}';`).join("\n");
...
const writeStaticClientFiles = async (ctx) => {
  ...
  await fs__default.default.writeFile(
    ...
    prettier.format(getEntryModule({ plugins: ctx.plugins }), {
      parser: "babel"
    })
  );
...
const createBuildContext = async ({
  ...
  const pluginsWithFront = Object.values(plugins).filter(filterPluginsByAdminEntry).map((plugin) => ({
    path: !plugin.isLocal ? `${plugin.pathToPlugin}/strapi-admin` : `${path__default.default.relative(runtimeDir, plugin.pathToPlugin)}/strapi-admin`,
    name: plugin.name,
...
const filterPluginsByAdminEntry = (plugin) => {
  ...
  try {
    const isLocalPluginWithLegacyAdminFile = syncFs__default.default.existsSync(
      //@ts-ignore
...
exports.index = index;
//# sourceMappingURL=index-cce8ecb6.js.map
